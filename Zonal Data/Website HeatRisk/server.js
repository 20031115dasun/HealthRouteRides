...
def optimize_route(start, end):
    ...
    for alt_lat, alt_lon in alternatives:
        alt_risk = risk_at_point(alt_lat, alt_lon)
        print(f"Alternative midpoint ({alt_lat}, {alt_lon}) risk: {alt_risk}")
        if alt_risk is not None and alt_risk != 'High':
            best_point = (alt_lat, alt_lon)
            best_risk = alt_risk
            break

    return [ (lat1, lon1), best_point, (lat2, lon2) ]

@app.route('/api/route-info', methods=['POST'])
def route_info():
    data = request.get_json()
    print("\nReceived data:", data)
    required = ['pickupCoords', 'dropoffCoords', 'date', 'time', 'transportType']
    missing = [f for f in required if f not in data]
    if missing:
        print(f"Missing fields: {missing}")
        return jsonify({"message": "Missing required parameters", "missing": missing}), 400
    try:
        travel_datetime = datetime.strptime(f"{data['date']} {data['time']}", "%Y-%m-%d %H:%M")
        print(f"Travel datetime: {travel_datetime}")
